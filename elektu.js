class Colours {
  ...
  getRandomColour() {
    return this.colour.splice(
      Math.floor(Math.random() * this.colour.length),
      1
    )[0];
  }
  add(colour) {
...
class PlayerTouch {
  ...
  computeOuterCircle() {
    if (
      this.outerCircleEndAngle - this.outerCircleStartAngle >= 2 * Math.PI ||
      this.state === this.touchState.onlySelected ||
      this.state === this.touchState.selected
    ) {
    ...
    switch (this.state) {
      ...
      default:
        ...
        if (this.timeoutStarted !== -1 && timestamp > this.timeoutStarted) {
          const totalAngle =
            ((timestamp - this.timeoutStarted) / this.timeoutDuration) *
            Math.PI *
            2;
          this.timeoutCircleStartAngle += 0.04;
          this.timeoutCircleEndAngle =
            this.timeoutCircleStartAngle + totalAngle;
        }
        break;
    }
  ...
  drawTouch(ctx) {
    ...
    ctx.arc(
      ...
      this.y,
      this.radius + 12,
      this.outerCircleStartAngle,
      this.outerCircleEndAngle
    );
...
class Elektu {
  ...
  add(x, y, id) {
    let colour =
      this.feature.type === this.feature.featureType.teams
        ? this.colours.getNoTeamColour()
        : this.colours.getRandomColour();
  ...
  touchesLength() {
    ...
    for (const touch of this.touches) {
      if (
        touch.state !== touch.touchState.deletion &&
        touch.state !== touch.touchState.obsolete
      ) {
        ++touchCount;
      }
    }
  ...
  resetTimerTrigger() {
    clearTimeout(this.timerTrigger);
    if (
      this.feature.shouldTimerStart(this.touchesLength(), this.selectedNumber)
    ) {
      this.timerTrigger = setTimeout(
  ...
  triggerSelection() {
    ...
    const shuffleTouchList = () => {
      ...
      this.touches.forEach((touch) => {
        console.log(touch);
        if (Number(touch.y) < target.y) {
          target.y = Number(touch.y);
        }
      });

      this.touches = this.touches.filter((touch) => {
        if (Number(touch.y) === Number(target.y)) {
          return true;
        } else {
          return false;
        }
    ...
    const selectTeams = (numberOfTeams) => {
      ...
      const splitTeams = splitIntoTeams(
